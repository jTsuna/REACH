                                ...
                                @foreach ($programs as $program)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                                            {{ is_null($program->user_id) ? '' : App\Models\User::where('id', $program->user_id)->first()->name }}
                                        </td>
                                        <td class="px-6 py-4 font-medium text-right text-md whitespace-nowrap">
                                            <button onClick="toggleElement('show{{ $program->id }}')"
                                                class="text-green-600 hover:text-green-900">Edit</button>
                                        </td>
                                        <td class="px-6 py-4 font-medium text-right text-md whitespace-nowrap">
                                            <form method="post" action="{{ route('users.destroy', $program->id) }}">
                                                @csrf
                                                @method('delete')
                                                <button type="submit"
                                    ...
                                    <div id="show{{ $program->id }}" class="fixed inset-0 z-10 hidden overflow-y-auto"
                                            ...
                                            <div
                                                ...
                                                <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <form method="POST"
                                                            action="{{ route('departments.update', $program->id) }}">
                                                            @csrf
                                                            @method('patch')
                                                            <div
